...
- 저장되어있는 JSON 파일의 형식([{}])문제일 수도 있다 
    - []는 빼고, 한 줄씩 dict 형식의 로그만 덧붙이는 식으로 대충 저장하면 될런지 잘 모르겠다.  
&nbsp;  

- EC2 ETL_Pipeline 4시간 이상 에러 없이 정상 작동한다. 파이프라인 베이스 모델 구축 완료.  
    ...
    - 시간마다 1200개의 로그 저장(5분 스케줄링, 압축 로그 파일크기:51.2KB ~ 51.6KB)
        - 시스템 과부화 예방, 안정성을 고려해 10분 스케줄링으로 변경
    - 한 시간에 한 번씩만 S3 연결,업로드 함수 호출.
    - 로컬 저장되는 데이터는 서버에서 받아온 원본 json로그(데이터 손실 대비, 확인용).
    - 받아오는 로그들을 압축하여 저장한 뒤, S3 적재 후에 삭제해서 추가된 데이터의 시간(h)을 반영함.
